...
// Quick function for running assertions and erroring out on failure
...
// Simulates have -Max based arrays
function transformIndex(max, index){
  if(index < 0){
    return max - (-index);
  } else {
    return index + max;
...
// The following two functions are a set of helper functions
// that help map the ideas presented from the paper into a programming
// language that does not let you have an idea of a negative index;

// These functions both assume that array is of length 2*(M+N)+1 [note this also means it's length is odd]
// and that the 0th index of this array is the middle element
function getFromEndpointsArray(array, max, index){
  return array[transformIndex(max, index)];
}

function insertIntoEndpointsArray(array, max, index, value){
  array[transformIndex(max, index)] = value;
}
...
(function testInsertIndex(){
  ...
  insertIntoEndpointsArray(v, max, 0, 20);
  assert(getFromEndpointsArray(v, max, 0) === 20);
  return "Success!";
})();
(function testTransformIndex(){
  let v = [];
  let max = 10;
  for(let i=-max; i<= max; i++){
    v.push(i);
  ...
  assert(v[transformIndex(10, -10)] === -10);
  assert(v[transformIndex(10, 0)] === 0);
  assert(v[transformIndex(10, 10)] === 10);
  return "Success!";
})();
...
function shortestEditSequence(first, second){
  ...
  insertIntoEndpointsArray(v, MAX, 1, 0);
  let history = [];
  ...
  for(let d=0; d <= MAX; d++){
    ...
    for(let k=-d; k <= d; k=k+2){
      ...
      let insertOrDelete = "";
      if (k === -d || (k !== d && getFromEndpointsArray(v, MAX, k - 1) < getFromEndpointsArray(v, MAX, k + 1))){
        insertOrDelete = "Insert";
        x = getFromEndpointsArray(v, MAX, k+1);
      } else {
        insertOrDelete = "Delete";
        x = getFromEndpointsArray(v, MAX, k-1)+1;
      }
...
function generateEditGraph(first, second, history, K, D){
  ...
  options.forEach(function([operation, position]){
    ...
    let x = getFromEndpointsArray(vatD, MAX, position);
    let y = x - position;
  ...
  if (bestOperation === "insert"){
    let x = getFromEndpointsArray(vatD, MAX, K+1);
    let y = x - (K+1);
