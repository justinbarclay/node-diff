...
function backward({first,
  ...
  return {x: x, y:y, u: xStart, v: yStart, reverse: true};
};
...
function middleSnake(first, N, second, M){
...
function determineAction({x, y, u, v, reverse}){
...
function translateCoordinates({x, y, u, v, reverse}, xOffset, yOffset){
  return {
    x: x + xOffset,
    y: y + yOffset,
    u: u + xOffset,
...
function diff(first, N, firstOffset, second, M, secondOffset){
  let actions = {insert: [],
                 delete: []};
  if (N > 0 && M > 0){
    let [difference, k, {x,y,u,v, reverse}] = middleSnake(first, N, second, M); // not sure why we need k yet
    ...
    let action = determineAction(translateCoordinates({x, y, u, v, reverse}, firstOffset, secondOffset));;
    if(action[0] !== null){
    ...
    if(difference > 1){
      let [_, forwardActions] = diff(first.slice(0, x), x, firstOffset, second.slice(0, y), y, secondOffset);
      actions = mergeActions(forwardActions, actions);
      let [_1, reverseActions] = diff(first.slice(u, N), N-u, (firstOffset + u), second.slice(v, M), M-v, (secondOffset + v));
...
function diffLinear(string1, string2){
  let [difference, editGraph] = diff(string1, string1.length, 0, string2, string2.length, 0);
  let simplifiedEditGraph = concatEditGraph(editGraph);
  return [difference, simplifiedEditGraph];
}
...
function prettyPrintString(string, simplifiedEdits, type){
  ...
  let edit = simplifiedEdits[type].shift();
  ...
  for(let i=0; i< string.length; i++){
    ...
    if(edit && i === edit["to"]) {
      newString += ENDCOLOUR;
      edit = simplifiedEdits[type].shift();
    }
  }
  return newString;
}
exports.middleSnake = middleSnake;
exports.diffLinear = diffLinear;
exports.prettyPrintString = prettyPrintString;
